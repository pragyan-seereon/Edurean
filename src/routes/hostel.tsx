...
type Block = { name: string; gender: "Boys" | "Girls" | "Co-ed"; warden: string; rooms: Room[] };

const SEED: Block[] = [
  { name: "Aravalli Block", gender: "Boys", warden: "Mr. Rao", rooms: [
  ...
  { name: "Vindhya Block", gender: "Girls", warden: "Mrs. Iyer", rooms: [
  ...
  return (
    <PageContainer>
      <PageHeader eyebrow="Operations" title="Hostel Management"
        ...
        actions={<Button size="sm" className="gradient-primary border-0" onClick={() => setAddBlock(true)}><Plus className="h-4 w-4" />Add Block</Button>}
      />

      <CrudDialog open={addBlock} onOpenChange={setAddBlock} title="Add Hostel Block"
        fields={[
          { name: "name", label: "Block Name" },
          { name: "gender", label: "Gender", type: "select", options: ["Boys", "Girls", "Co-ed"] },
          { name: "warden", label: "Warden Name" },
        ]}
        submitLabel="Add Block"
        onSubmit={(d) => setBlocks((p) => [...p, { name: String(d.name), gender: String(d.gender) as Block["gender"], warden: String(d.warden), rooms: [] }])}
      />
            ...
            <Card key={b.name} className="border-border/60">
              <CardHeader className="flex-row items-center justify-between space-y-0 pb-3">
                <div>
                  ...
                  <CardDescription>Warden: {b.warden} · {b.rooms.length} rooms · {b.rooms.reduce((s, r) => s + r.allocations.length, 0)}/{b.rooms.reduce((s, r) => s + r.capacity, 0)} beds</CardDescription>
                </div>
