...
function InstituteDetailPage() {
  ...
  const impersonate = () => {
    ...
    auth.updateProfile({ institute: inst.name, designation: `Admin · ${inst.name}` });
    toast.success(`Impersonating Admin of ${inst.name}`, { description: "You can now see and manage everything as their admin would." });
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button variant="outline" size="sm" onClick={impersonate}><ExternalLink className="h-4 w-4" />Impersonate Admin</Button>
