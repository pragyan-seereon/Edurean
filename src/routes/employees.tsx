...
import { Search, Plus, Download, MoreHorizontal, Users, UserCheck, Briefcase, Award, Pencil, Trash2, Eye } from "lucide-react";
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        actions={
          <>
            <Button variant="outline" size="sm" onClick={() => toast.success("CSV exported")}><Download className="h-4 w-4" />Export</Button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <KpiCard label="Teaching Faculty" value={employees.filter((e) => e.role.includes("Teacher") || e.role.includes("Principal")).length.toString()} icon={<Award className="h-5 w-5" />} tone="info" />
        <KpiCard label="Open Positions" value="6" icon={<Briefcase className="h-5 w-5" />} tone="warning" />
      </div>
        ...
        <CardContent className="p-0">
          ...
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {filtered.length === 0 && (
                  <TableRow><TableCell colSpan={8} className="text-center text-sm text-muted-foreground py-10">No employees match your filters.</TableCell></TableRow>
                )}
                {filtered.map((e) => (
